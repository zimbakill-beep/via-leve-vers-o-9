...
def go_to(step: int):
    st.session_state.step = max(0, min(7, step))  # agora 8 etapas (0..7)
...
# -------- Etapa 5 --------
    ...
    if status != "potencialmente_elegivel":
        ...
        st.warning("ℹ️ **Obrigado por responder!** Neste momento, precisamos de uma **avaliação médica** antes de seguir com a prescrição.")
        if reasons:
        ...
        st.subheader("6) Escolha seu plano ViaLeve 💡")
...
# -------- Etapa 6 --------
    ...
    if plano not in ["entrega","premium"]:
        ...
        st.subheader("7) Preferência de medicação 💊")
...
# -------- Etapa 7 --------
    ...
    st.subheader("8) Resumo e pagamento ✅")
...
7. **Assinatura eletrônica:** meu aceite eletrônico tem validade jurídica.
    ...
    with st.form("consent_checkout"):
        ...
        with colC:
            b_pay = st.form_submit_button("Ir para pagamento 💳", use_container_width=True, disabled=not st.session_state.consent_ok)
        ...
        if b_pay:
            ...
            if checkout_url:
                st.link_button("Abrir checkout", checkout_url, use_container_width=True)
            else:
...
st.markdown("---")
st.caption("ViaLeve • Protótipo v0.11 — PT-BR • Streamlit (Python)")