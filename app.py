...
def normalize_nome(s: str) -> str:
    return " ".join(p.capitalize() for p in s.strip().split() if p)

def parse_altura_pt(texto: str) -> Tuple[Optional[float], Optional[str]]:
    s = (texto or "").strip().replace(",", ".")
    try:
        v = round(float(s), 2)
        if 1.20 <= v <= 2.30:
            return v, None
        return None, "Altura fora da faixa esperada (1,20–2,30 m)."
    except:
        return None, "Altura inválida. Use formato 1,70."

def yesno(v): 
    return "Sim" if str(v).lower() in ["sim","true","1"] else "Não"

def build_resumo_md(a: Dict[str, Any]) -> str:
    linhas = [
        "# ViaLeve — Resumo do questionário",
        "## Dados pessoais",
        f"- Nome: {a.get('nome','—')}",
        f"- E-mail: {a.get('email','—')}",
        f"- Nascimento: {a.get('data_nascimento','—')}",
        f"- Identidade: {a.get('identidade','—')}",
        f"- Celular: {a.get('telefone','—')}",
        f"- WhatsApp: {'Sim' if a.get('whatsapp_ok') else 'Não'}",
        "",
        "## Saúde e medidas",
        f"- Peso: {a.get('peso','—')} kg",
        f"- Altura: {a.get('altura','—')} m",
        f"- Comorbidades: {'Sim' if a.get('tem_comorbidades')=='sim' else 'Não'}",
        f"- Quais: {a.get('comorbidades','—')}",
        "",
        "## Condições clínicas",
        f"- Gravidez: {yesno(a.get('gravidez'))}",
        f"- Amamentando: {yesno(a.get('amamentando'))}",
        f"- Oncológico ativo: {yesno(a.get('tratamento_cancer'))}",
        ...
        f"- Corticoide crônico: {yesno(a.get('uso_corticoide'))}",
        f"- Antipsicóticos: {yesno(a.get('antipsicoticos'))}",
        f"- Alergias (componentes): {', '.join(a.get('alergias_componentes', [])) or '—'}",
        f"- Outras alergias: {a.get('outros_componentes','—')}",
        "",
        "## Histórico e objetivo",
        f"- Já usou medicação: {yesno(a.get('usou_antes'))}",
        f"- Quais: {', '.join(a.get('quais', [])) or '—'}",
        f"- Efeitos: {a.get('efeitos','—')}",
        f"- Objetivo: {a.get('objetivo','—')}",
        f"- Pronto para mudar (0–10): {a.get('pronto_mudar','—')}",
    ]
    if a.get("plano"):
        linhas += ["", "## Plano escolhido", f"- Plano: {a.get('plano').title()}"]
        if a.get("med_pref"):
    ...
    if st.session_state.step == 0:
        ...
        with st.form("step0"):
            ...
            with col1:
                ...
                telefone = st.text_input("Celular (WhatsApp) — opcional", value=st.session_state.answers.get("telefone", ""), placeholder="(11) 91234-5678")
                whatsapp_ok = st.checkbox("Quero receber contato pelo WhatsApp", value=st.session_state.answers.get("whatsapp_ok", False))
            with col2:
                hoje = date.today()
            ...
            if b_cont:
                ...
                if altura_err:
                    ...
                    st.session_state.answers.update({"peso": int(peso),
                                                     "altura": altura_val,
                                                     "tem_comorbidades": ("sim" if tem_comorbidades=="Sim" else "nao"),
